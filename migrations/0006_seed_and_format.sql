-- Insert the post if it does not already exist (by title), then update with formatted content

INSERT INTO posts (title, content, tags, metadata, created_at, updated_at)
SELECT 'Secure Secrets Management', 'Seed placeholder', JSON_ARRAY('Security','DevOps','Cloud','Vault','SecretsManagement','BestPractices'), JSON_OBJECT('author','Puspo','category','Cybersecurity','read_time','5 min'), NOW(), NOW()
WHERE NOT EXISTS (
  SELECT 1 FROM posts WHERE title = 'Secure Secrets Management'
);
